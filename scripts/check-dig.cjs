...
module.exports=async function checkDig(win,mouse,waitFor,pause) {
  ...
  await pause(100);
  assert.equal(await js(`document.querySelectorAll('.dig-dust').length`),0,'No dust before the first scrape');
  await waitFor(win,`document.querySelectorAll('.dig-dust[data-paw="0"]').length===3`);
  const firstOrigin=await js(`parseFloat(document.querySelector('.dig-dust').style.left)`);
  assert.ok(await js(`(()=>{const dots=[...document.querySelectorAll('.dig-dust')];return new Set(dots.map(d=>d.style.left)).size===1 && dots.every(d=>parseFloat(d.style.left)>x+petWidth/2&&parseFloat(d.style.top)>y+petWidth*.78&&parseFloat(d.style.top)<y+petWidth*.9&&parseFloat(d.style.getPropertyValue('--drift'))<0)})()`),'Only the paw scraping the ground kicks dust backward');
  await waitFor(win,`document.querySelectorAll('.dig-dust[data-paw="1"]').length===3`);
  assert.ok(await js(`document.querySelectorAll('.dig-dust[data-paw="0"]').length===0&&parseFloat(document.querySelector('.dig-dust').style.left)>${firstOrigin}`),'The next scrape comes from the opposite front paw');
  ...
  await waitFor(win,`document.querySelectorAll('.dig-dust').length===3`);
  assert.ok(await js(`(()=>{const dots=[...document.querySelectorAll('.dig-dust')];return new Set(dots.map(d=>d.style.left)).size===1&&dots.every(d=>parseFloat(d.style.left)<x+petWidth/2&&parseFloat(d.style.getPropertyValue('--drift'))>0)})()`),'Dust follows tiny pet size and mirrored paws');
  await js(`setAction('idle');overrideTime=0;modeTimer=2;direction=1`);
  console.log('PASS alternating paw scrapes, separate timed dust, stationary body, hold cancellation and mirrored baby size');
};
